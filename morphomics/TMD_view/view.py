...
from morphomics.Topology.methods import filtr_function
from morphomics.Topology.methods import tree_to_property_barcode as tp_barcode
from morphomics.utils import tree_type, term_dict
from morphomics.TMD_view import common as cm
from morphomics.TMD_view import plot
from morphomics.TMD_view.common import blues_map
...
def trunk(tr, plane="xy", new_fig=True, subplot=False, hadd=0.0, vadd=0.0, N=10, **kwargs):
    ...
    treecolor = cm.get_color(_get_default("treecolor", **kwargs), tree_type[tr.get_type()])
...
def tree(tr, plane="xy", new_fig=True, subplot=False, hadd=0.0, vadd=0.0, **kwargs):
    ...
    if tr.get_type() not in tree_type:
        treecolor = "black"
    else:
        treecolor = cm.get_color(_get_default("treecolor", **kwargs), tree_type[tr.get_type()])
...
def tree_barcode_colors(tr, plane="xy", feature="path_distances", cmap=cm.jet_map):
    ...
    ph, ph_graph = tp_barcode(tr, filtration_function=filtr_function(feature))
