...
class Soma(object):
    ...
    from morphomics.Soma.methods import get_center
    from morphomics.Soma.methods import get_diameter

    def __init__(self, x=np.array([]), y=np.array([]), z=np.array([]),
                 d=np.array([])):
        """
        Constructor for morphomics Soma Object
