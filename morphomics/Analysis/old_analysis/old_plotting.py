...
def Plotting(self):
        ...
        Essential parameters:
            ...
            conditions_filepath (list, str or 0): if these are not 0, must point to the location of the morpho_infoframe that corresponds to each element of `Coordinate_filepath`
        ...
        for _idx in range(len(params["label_prefixes"])):
            ...
            # get coordinates and label/conditions of data points

            # check if reductioninfo file is given
            if params["ReductionInfo_filepath"][_idx] == 0:
                coordinates = morphomics.utils.load_obj(
                    params["coordinate_key_filepath"][_idx].replace(".pkl", "")
                )
                morpho_info = morphomics.utils.load_obj(
                    params["conditions_filepath"][_idx].replace(".pkl", "")
                )
                
            elif os.path.exists(params["ReductionInfo_filepath"][_idx]):
            ...
            coordinates = np.vstack(np.array(coordinates))

            # get color for different labels/conditions
            color_conditions = [
                conds
            ...
            morphomics.plotting.scatterplot_3D_conditions(
                coordinates, morpho_info, color_conditions, colormap, params["label_prefixes"][_idx]
            )
    ...
    for _idx_conds in range(len(_colormap)):
        ...
        if len(time_inds) > 0:
            scatterplot_3D(
                ...
                _size = 2,
                _color = _c,
                _description = "%s%s" % (_label_prefix, "_".join(conds.values)),
                _alpha = 0.7,
            )
